...
# import
xzcat -c database-latest.sql.xz | mysql --defaults-file=~/.mysql_credentials orawelln_wp && mysql --defaults-file=~/.mysql_credentials --execute="USE orawelln_wp; UPDATE wpora_options SET option_value = 'http://orawellness.dev' WHERE option_value = 'http://orawellness.pressedsolutions.com';" && mv database-latest.sql.xz ~/.Trash/
