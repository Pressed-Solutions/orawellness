...
function ora_tagged_posts_loop() {
    if ( get_field( 'post_tag' ) ) {
        ...
        if ( $tagged_posts_query->have_posts() ) {
            ...
            <h2 class="alternate" id="related-posts">Recent Posts <em>about</em> ' . esc_attr( get_field( 'post_tag' )->name ) . '</h2>';
            ...
            custom_pagination( $tagged_posts_query->max_num_pages, "", $paged, '#related-posts' );
            echo '</section>';
        }
